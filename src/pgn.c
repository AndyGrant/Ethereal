...
static uint16_t parse_san(Board *board, const char *SAN) {
    ...
    return moveIsLegal(board, move) ? move : NONE_MOVE;
}


static unsigned pgn_read_until_move(char *buffer, unsigned index) {
    for (; !isalpha(buffer[index]) && buffer[index] != '\0'; index++);
    return index;
}

static unsigned pgn_read_until_begin_comment(char *buffer, unsigned index) {
    for (; buffer[index] != '{'; index++);
    return index;
}

static unsigned pgn_read_until_end_comment(char *buffer, unsigned index) {
    for (; buffer[index] != '}'; index++);
    return index;
}
...
static bool pgn_read_moves(FILE *pgn, PGNData *data, Board *board) {
    ...
    while (1) {
        ...
        // Read, Save, and Apply the next move if there is one
        index = pgn_read_until_move(data->buffer, index);
        if (data->buffer[index] == '\0') return false;
        data->moves[data->plies] = parse_san(board, data->buffer + index);
        applyMove(board, data->moves[data->plies], &undo);
        ...
        if (sscanf(data->buffer + index + 1, "%lf", &eval) == 1)
            data->evals[data->plies] = round(fmin(fmax(100.0 * eval, -MATE_IN_MAX), MATE_IN_MAX));

        // Otherwise, assume we had a mate score like -MABC
        else {
            int plies = atoi(data->buffer + index + 2);
            data->evals[data->plies] = data->buffer[index] == '+' ? MATE - plies : -MATE + plies;
        }

        // Always save the eval as White's Point of View
        if (board->turn == BLACK)
            data->evals[data->plies] = -data->evals[data->plies];
...
static bool process_next_pgn(FILE *pgn, PGNData *data, Board *board) {
    ...
    // Read Result & Fen and skip to Moves
    while (pgn_read_moves(pgn, data, board));
