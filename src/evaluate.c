...
const int PawnValue[PHASE_NB] = {  86,  87};


const int PawnIsolated[PHASE_NB] = {   0,  -8};

const int PawnStacked[PHASE_NB] = { -16, -21};

const int PawnBackwards[2][PHASE_NB] = { {   1,   0}, { -11, -13} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   8,   1}, {  10,   2}, {   7,   6}, {  13,  13},
    {   7,   1}, {  14,   5}, {   9,   5}, {  16,  10},
    {  12,   6}, {  18,  12}, {  19,  12}, {  25,  16},
    {  24,   0}, {  37,  11}, {  71,  29}, {  62,  34},
    {  82, -50}, { 112, -44}, {  85, -40}, { 113,   4},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightValue[PHASE_NB] = { 299, 278};



const int KnightAttackedByPawn[PHASE_NB] = { -34, -30};

const int KnightOutpost[2][PHASE_NB] = { {  14, -19}, {  32,  10} };
...
const int BishopValue[PHASE_NB] = { 301, 281};



const int BishopWings[PHASE_NB] = {  -3,  -5};

const int BishopPair[PHASE_NB] = {  25,  41};

const int BishopAttackedByPawn[PHASE_NB] = { -33, -29};

const int BishopOutpost[2][PHASE_NB] = { {  15,  -9}, {  35,  -5} };

const int BishopMobility[14][PHASE_NB] = {
    { -65, -72}, { -38, -49}, { -16, -28}, {  -4, -13},
    {   6,  -4}, {  14,   5}, {  18,  12}, {  21,  11},
    {  24,  15}, {  24,  10}, {  24,  13}, {  23,  -6},
    {  21,  10}, { -14, -39},
};

const int RookValue[PHASE_NB] = { 415, 457};



const int RookFile[2][PHASE_NB] = { {   7,   0}, {  25,  -1} };

const int RookOnSeventh[PHASE_NB] = {   6,   8};

const int RookMobility[15][PHASE_NB] = {
    {-120, -96}, { -50, -76}, { -13, -46}, {  -6, -19},
    {  -5,  -7}, {  -6,   3}, {  -6,  13}, {  -3,  19},
    {   1,  25}, {   9,  28}, {  12,  34}, {  17,  39},
    {  17,  41}, {  15,  35}, {  11,  28},
};

const int QueenValue[PHASE_NB] = { 828, 862};

const int QueenChecked[PHASE_NB] = { -50, -29};

const int QueenCheckedByPawn[PHASE_NB] = { -60, -37};



const int QueenMobility[28][PHASE_NB] = {
    {-197, -66}, { -86,-316}, { -74,-141}, { -40, -93},
    { -27,-105}, { -25, -50}, { -18, -51}, { -15, -52},
    { -12, -47}, {  -8, -39}, {  -5, -28}, {  -2, -18},
    {   0, -11}, {   0,  -2}, {   2,   1}, {   3,  12},
    {   2,  18}, {   2,  20}, {   2,  21}, {   6,  28},
    {   5,  26}, {  12,  26}, {  20,  28}, {  25,  24},
    {  23,  24}, {  21,  12}, {  33,  36}, {   2,  17},
};
...
const int KingDefenders[12][PHASE_NB] = {
    { -15,   8}, { -15,  -3}, {  -1,  -1}, {   6,   1},
    {  17,   1}, {  25,   7}, {  35,  -5}, {  48,  34},
    {   8,   4}, {   8,   4}, {   8,   4}, {   8,   4},
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{  -4,  -7}, {  11, -11}, {  18,  -4}, {   8, -10}, {  17,  -8}, {  -9, -11}, {  68,  23}, { -18,  15}},
   {{  -2,   2}, {  12,  -4}, {  24,  -6}, {  -2,  -9}, {   4, -18}, { -75,  20}, {-105, -14}, { -16,   2}},
   {{   7,   0}, {   6,  -1}, { -10,   0}, {  -5,  -4}, {   2,  -4}, {  46, -24}, {  38, -42}, {  -6,   0}},
   {{  13,  12}, {   9,  -1}, {  -3,  -5}, {  12,  -3}, {   9, -10}, { -17, -21}, { -13,  24}, {   0,  -1}},
   {{   0,  10}, {   0,   4}, { -13,   1}, { -13, -10}, { -18, -22}, { -27, -39}, {  40, -25}, { -10,  -1}},
   {{   7,   0}, {  12,   2}, {  -3,   0}, {  -7, -12}, {  10, -22}, {  -1,  -2}, {  10,   1}, { -11,   0}},
   {{   3,  -2}, {   1,  -8}, { -13,  -6}, { -16, -11}, { -22, -20}, { -30,   2}, {  50,  13}, { -17,   8}},
   {{ -11,  -9}, {   2,  -5}, {   3,   0}, {  -1,   2}, { -17,  -4}, {  -8,  30}, {   6,  43}, { -15,  11}}},
  {{{   0,   0}, { -12, -15}, {   5, -14}, { -35,   0}, { -16,  25}, { -29,  93}, { -10, -57}, { -32,  11}},
   {{   0,   0}, {  16,   1}, {  17,  -4}, { -13,   1}, {   6,   0}, { -99,  -8}, {-269,  10}, { -23,   6}},
   {{   0,   0}, {  17,   0}, {   4,  -6}, {  18, -11}, {  13,  -3}, { -87, -31}, { -83, -89}, {  -7,   0}},
   {{   0,   0}, {   4,   2}, {  -2,  15}, { -18,  -8}, { -13, -10}, {  -6,   0}, { -31,   9}, { -14,  -4}},
   {{   0,   0}, {   1,   9}, {   7,   1}, {   8,  -9}, {   5, -16}, { -17, -15}, { -42,  16}, {  -2,  -4}},
   {{   0,   0}, {   0,   1}, {  -6,   0}, { -11,  -8}, {   4, -11}, { -42,   0}, { -48,  78}, { -15,   0}},
   {{   0,   0}, {   8,  -2}, {  -1,   0}, {  -7,  -7}, { -19,  -7}, {  -6,  26}, {  23, -67}, { -28,   8}},
   {{   0,   0}, {   3, -12}, {   6,  -8}, { -35,  -2}, { -17,  21}, { -10, -14}, {-112, -58}, { -28,   8}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -24, -11}, { -22,  -4}, { -19, -14}, {  16, -11}, {  42, -16}, {  94,   7}, {   0,   0}},
   {{   0,   0}, {  -4,   0}, { -12,  12}, { -10,  21}, {  10,  26}, {  58,  32}, { 115,  58}, {   0,   0}}},
  {{{   0,   0}, {  -3,   4}, { -12,   1}, {  -8,  11}, {  21,  16}, {  71,  40}, { 137,  87}, {   0,   0}},
   {{   0,   0}, {   1,   7}, {  -5,  11}, {  -9,  35}, {   5,  75}, {  52, 152}, { 129, 238}, {   0,   0}}},
};
