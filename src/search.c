...
int LMRTable[64][64];
int LateMovePruningCounts[2][10];
...
void initSearch() {
    ...
    for (int depth = 1; depth < 64; depth++)
        for (int played = 1; played < 64; played++)
            LMRTable[depth][played] = 0.8162 + log(depth) * log(played) / 2.328;

    for (int depth = 1; depth <= 9; depth++) {
        LateMovePruningCounts[0][depth] = 2.3195 + 0.4466 * depth * depth;
        LateMovePruningCounts[1][depth] = 4.0722 + 0.8540 * depth * depth;
    }
}
...
int search(Thread *thread, PVariation *pv, int alpha, int beta, int depth, bool cutnode) {
    ...
    if (   !PvNode
        ...
        R = 5 + depth / 5 + MIN(3, (eval - beta) / 190) + (ns-1)->tactical;
